...
It holds a connection to the relay and exits the moment a request arrives, printing the request (already claimed). Muse's runtime delivers that output into a new turn. After replying, Muse starts `tincan wait &` again. Transient network errors are retried with backoff, so a flaky path does not end the wait.
