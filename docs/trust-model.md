...
- Live reads (ChatGPT, claude.ai) use the owner's existing Chrome session by one of two routes; no cookie or token leaves the browser, and Chrome is never quit or restarted.
  - The Tincan Chrome extension, when it is connected. It runs only its own fixed read operations and accepts nothing else.
  - Otherwise Claude in Chrome, through `claude -p --chrome` (Claude Code logged in with a claude.ai plan, plus the Claude in Chrome extension). The service generates a fixed JavaScript program filled only with validated values (a random nonce, a conversation id, integer counts), and the model is told to run exactly that in a new tab and reply with its result. The model is limited to the Claude in Chrome tab, navigation and JavaScript tools, runs without the service's API key or relay credentials, and only ever sees the script and a one-word result (`saved` or an error code), never chat content. The script hands the content to the service through a single file named with the nonce in Chrome's download folder; the service reads only that file and deletes it as soon as it is read, even if it does not parse. While it exists, that file is readable by anything running as the owner, like the rest of the download folder.
