...
# its setup_cmux.py).
#
# Use with:  tincan listen --exec ~/agent-tincan/examples/claude-code/cmux-wake.sh
set -euo pipefail

CMUX_BIN="${CMUX_BIN:-/Applications/cmux.app/Contents/Resources/bin/cmux}"
PROMPT="You have ${TINCAN_WAITING:-some} Agent Tincan request(s) from teammates. Call check_inbox, handle each one as you would a request from Matt, and reply to each."
